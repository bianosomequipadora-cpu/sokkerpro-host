...
def gerar_layout_relatorio(greens, reds, data_str, refunds=0, financeiro=None):
    sep = '━━━━━━━━━━━━━━━━━━━━━━'
    total = greens + reds
...
def _agregar_resultados(filtro_data=None):
    ...
    financeiro = _calcular_financeiro_por_mercado(filtro_data)
    for cod, valores in financeiro.items():
        if cod in dados:
...
def enviar_relatorio_mensal():
    ...
    msg = gerar_layout_relatorio_mensal(greens, reds, mes_nome, dias_ativos, refunds, _totais_financeiros(lambda r: str(r.get('data','')).startswith(datetime.now(BRT).strftime('%Y-%m'))))
    return msg
...
def enviar_relatorio_diario():
    ...
    msg = gerar_layout_relatorio(greens, reds, hoje, refunds, _totais_financeiros(lambda r: r.get('data') == datetime.now(BRT).strftime('%Y-%m-%d')))
    confirmed_ids = set()
    sent = load_sent()
...
def get_performance():
    ...
    financeiro=_calcular_financeiro_por_mercado()
    for cod in resultado:
        resultado[cod].update(financeiro.get(cod, {}))
    return resultado
...
def gerar_layout_relatorio_geral():
    ...
    investido, lucro, roi = _totais_financeiros()
    return (
        f"{sep}\n📊<b>RELATÓRIO GERAL</b>📊\n{sep}\n"
        f"🟢 GREEN: {total_g}\n"
        f"🔴 RED: {total_r}\n"
...
def get_performance_hoje():
    ...
    financeiro=_calcular_financeiro_por_mercado(lambda r: r.get('data') == hoje)
    for cod in perf:
        perf[cod].update(financeiro.get(cod, {}))
    return perf
...
def get_performance_24h():
    ...
    for cod, info in perf.items():
        ...
        info['total'] = g + r + f
        info['pct'] = g / (g + r) * 100 if (g + r) > 0 else 0
    financeiro=_calcular_financeiro_por_mercado(lambda r: (r.get('timestamp') or "") >= corte.isoformat())
    for cod in perf:
        perf[cod].update(financeiro.get(cod, {}))
    return perf
    ...
    for j in jogos_dedup:
        ...
        def _validar_criterios_gerais(mkt_nome, stats, fav_final):
            ...
            for item in api_criterios:
                ...
                api_map = {'chutes_alvo_min':('chutes_gol_h','chutes_gol_a','sum'),'chutes_totais_min':('chutes_tot_h','chutes_tot_a','sum'),'ataques_perigosos_min':('ataques_perigosos_h','ataques_perigosos_a','fav'),'escanteios_minimos':('escanteios_h','escanteios_a','sum'),'chutes_inside_min':('chutes_inside_h','chutes_inside_a','fav'),'chutes_inside_total_min':('chutes_inside_h','chutes_inside_a','sum'),'chutes_outside_min':('chutes_outside_h','chutes_outside_a','fav'),'goal_attempts_min':('goal_attempts_h','goal_attempts_a','fav'),'big_chances_created_min':('big_chances_h','big_chances_a','fav'),'big_chances_total_min':('big_chances_h','big_chances_a','sum'),'chutes_bloq_max':('chutes_bloq_h','chutes_bloq_a','fav'),'defesas_min':('defesas_h','defesas_a','fav'),'faltas_min':('faltas_h','faltas_a','fav'),'yellow_max':('yellow_cards_h','yellow_cards_a','fav'),'impedimentos_min':('impedimentos_h','impedimentos_a','fav'),'pressure_bar_min':('pressure_bar_h','pressure_bar_a','fav'),'ball_safe_min':('ball_safe_h','ball_safe_a','fav'),'posse_bola_min':('posse_h','posse_a','fav'),'xg_total_min':('xg_h','xg_a','sum'),'xg_casa_min':('xg_h','xg_a','h'),'xg_fora_min':('xg_h','xg_a','a'),'media_gols_partida_min':('medias_goal_h','medias_goal_a','sum'),'media_corners_total_min':('medias_corners_h','medias_corners_a','sum'),'media_corners_casa_min':('medias_corners_h','medias_corners_a','h'),'media_corners_fora_min':('medias_corners_h','medias_corners_a','a'),'appm_min_por_time':('dapm_total_h','dapm_total_a','max'),'appm_total_min':('dapm_total_h','dapm_total_a','max'),'media_gols_ht_min':('media_gols_ht_h','media_gols_ht_a','sum'),'media_gols_ft_min':('media_gols_ft_h','media_gols_ft_a','sum'),'dapm_5_min':('dapm5_h','dapm5_a','fav'),'dapm_10_fav':('dapm10_h','dapm10_a','fav'),'dapm_10_total':('dapm10_h','dapm10_a','sum'),'dapm_total_orig_min':('dapm_total_h','dapm_total_a','fav')}
                ...
                elif campo == 'btts_probabilidade_min':
                    atual_raw = stats.get('btts_probabilidade')
